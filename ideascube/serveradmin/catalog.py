...
def load_from_basepath(basepath):
    ...
    try:
        ...
        if json_parsing_error is not None:
...
class MissingPackageMetadata(Exception):
...
class Zim(BaseZim, typename='zim'):
    def install(self, download_path, install_dir, keep_downloads=False):
        ...
        if not keep_downloads:
            shutil.move(download_path, dest_name)
        else:
...
class Catalog:
    ...
    def _get_package(self, id, source):
        ...
        try:
            ...
            raise MissingPackageMetadata(id, 'type')
    ...
    def list_problems(self, ids):
        ...
        for pkgid, metadata in source.items():
            ...
            except (InvalidPackageType, MissingPackageMetadata) as e:
                errors.append(e)
