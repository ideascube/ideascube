...
def get_database(model_name, default=None):
    if model_name.startswith('search_idx_'):
        return 'transient'
    return default
...
class DatabaseRouter(object):
    ...
    def db_for_read(self, model, **hints):
        ...
        """
        return get_database(model._meta.model_name)
    ...
    def allow_relation(self, obj1, obj2, **hints):
        ...
        return get_database(obj1._meta.model_name) \
            == get_database(obj2._meta.model_name)

    def allow_migrate(self, db, app_label, model_name=None, **hints):
        """
        Allow migration only if the migration is applying to the right db.
        """
        db_to_use = get_database(model_name, default='default')
        if db_to_use != db:
            return False
