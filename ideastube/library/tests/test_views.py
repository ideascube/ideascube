...
def test_import_from_isbn(staffapp, monkeypatch):
    ...
    monkeypatch.setattr('ideastube.library.utils.load_cover_from_url',
                        lambda x: 'xxx')
    monkeypatch.setattr('ideastube.library.utils.read_url', lambda x: doc)
...
def test_import_from_files(staffapp, monkeypatch):
    monkeypatch.setattr('ideastube.library.utils.read_url', lambda x: None)
    form = staffapp.get(reverse('library:book_import')).forms['import']
    form['from_files'] = Upload('ideastube/library/tests/data/moccam.csv')
    response = form.submit()
    response.follow()
    assert Book.objects.count() == 2
...
def test_import_from_files_does_not_duplicate(staffapp, monkeypatch):
    monkeypatch.setattr('ideastube.library.utils.read_url', lambda x: None)
    path = 'ideastube/library/tests/data/moccam.csv'
    with open(path) as f:
        isbn = f.read().split('\t')[0]
...
def test_import_from_files_load_cover_if_exists(staffapp, monkeypatch):
    ...
    image = 'ideastube/tests/data/the-prophet.jpg'
    monkeypatch.setattr(
        'ideastube.library.utils.read_url',
...
def test_cannot_create_digital_specimen_when_serial_and_file_both_set(staffapp,
    ...
    assert not book.specimens.count()
    form['serial'] = '123456'
    form['file'] = Upload('ideastube/library/tests/data/test-digital')
    form.submit()
    assert not book.specimens.count()
