...
BACKUPS_ROOT = 'ideastube/serveradmin/tests/backups'
DATA_ROOT = 'ideastube/serveradmin/tests/data'
BACKUPED_ROOT = 'ideastube/serveradmin/tests/backuped'
...
def test_create_tarfile(monkeypatch, settings, format, extension):
    ...
    monkeypatch.setattr('ideastube.serveradmin.backup.Backup.ROOT',
                        BACKUPS_ROOT)
    monkeypatch.setattr('ideastube.serveradmin.backup.Backup.FORMAT', format)
    monkeypatch.setattr('ideastube.serveradmin.backup.make_name',
                        lambda: filename)
    proof_file = os.path.join(settings.BACKUPED_ROOT, 'backup.me')
    open(proof_file, mode='w')
